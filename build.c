...
static void sources(build_command_s* const command)
{
	...
	build_vector_append(command, "./source/lasm/archs/z80_parser.c");
...
{
	build_command_s command = {0};
	build_command_append(&command, "./build/debug/"PROJECT_NAME, "build", "-a", "z80", "-f", "elf", "./examples/experiments.lasm");
	const bool_t status = build_proc_run_sync(&command);
	build_vector_drop(&command);
	return status;
}
...
{
	build_command_s command = {0};
	build_command_append(&command, "./build/release/"PROJECT_NAME, "build", "-a", "z80", "-f", "elf", "./examples/experiments.lasm");
	const bool_t status = build_proc_run_sync(&command);
	build_vector_drop(&command);
	return status;
}
